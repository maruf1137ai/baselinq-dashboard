...
function Field({ label, children, colSpan }: { label: string; children: React.ReactNode; colSpan?: boolean }) {
  return (
    <div className={cn("flex flex-col gap-1.5", colSpan && "md:col-span-2")}>
      <label className="text-xs font-normal text-muted-foreground normal-case ml-0.5">{label}</label>
      {children}
    </div>
  );
}
...
const MODAL_INPUT_CLS = "w-full px-4 py-3 bg-muted/50 border border-transparent rounded-xl text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#6c5ce7]/20 focus:border-[#6c5ce7]/30 focus:bg-card transition-all";
          ...
          <SectionCard title="Organisation & Entity Details" subtitle="Corporate profile and registration information" icon={<Building2 className="h-4 w-4" />}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              <Field label="Company / Entity Name">
                <Input readOnly={!canEdit} value={formData.organization.name} onChange={e => setFormData({ ...formData, organization: { ...formData.organization, name: e.target.value } })} className={cn(INPUT_CLS, "font-normal", !canEdit && "bg-muted/50 cursor-not-allowed")} />
              </Field>
              <Field label="Company Registration No.">
                <Input readOnly={!canEdit} value={formData.organization.company_reg_number} onChange={e => setFormData({ ...formData, organization: { ...formData.organization, company_reg_number: e.target.value } })} className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")} />
              </Field>
              <Field label="VAT Registration Number">
                <Input readOnly={!canEdit} value={formData.organization.vat_number} onChange={e => setFormData({ ...formData, organization: { ...formData.organization, vat_number: e.target.value } })} className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")} />
              </Field>
              <Field label="CK Number">
                <Input readOnly={!canEdit} value={formData.organization.ck_number} onChange={e => setFormData({ ...formData, organization: { ...formData.organization, ck_number: e.target.value } })} className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")} />
              </Field>
              <Field label="Enterprise Size">
                <select disabled={!canEdit} value={formData.organization.company_size} onChange={e => setFormData({ ...formData, organization: { ...formData.organization, company_size: e.target.value } })} className={cn(INPUT_CLS, "w-full outline-none px-3", !canEdit && "bg-muted/50 cursor-not-allowed")}>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
            <Field label="Certificate Document">
              ...
              <p className="text-xs text-muted-foreground mt-1">PDF, JPG or PNG. Saved when you click Save Changes.</p>
            </Field>
            <Field label="Expiry Date">
              <Input
                ...
                className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")}
              />
            </Field>
          </div>
        </SectionCard>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
            <Field label="Bank Name">
              <Input readOnly={!canEdit} value={formData.banking_details.bank_name} onChange={e => setFormData({ ...formData, banking_details: { ...formData.banking_details, bank_name: e.target.value } })} className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")} placeholder="e.g. First National Bank" />
            </Field>
            <Field label="Branch Name">
              <Input readOnly={!canEdit} value={formData.banking_details.branch_name} onChange={e => setFormData({ ...formData, banking_details: { ...formData.banking_details, branch_name: e.target.value } })} className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")} placeholder="e.g. Sandton City" />
            </Field>
            <Field label="Branch Code">
              <Input readOnly={!canEdit} value={formData.banking_details.branch_code} onChange={e => setFormData({ ...formData, banking_details: { ...formData.banking_details, branch_code: e.target.value } })} className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")} placeholder="e.g. 250655" />
            </Field>
            <Field label="Account Number">
              <Input readOnly={!canEdit} value={formData.banking_details.account_number} onChange={e => setFormData({ ...formData, banking_details: { ...formData.banking_details, account_number: e.target.value } })} className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")} placeholder="e.g. 62012345678" />
            </Field>
            <Field label="Account Type">
              <Input readOnly={!canEdit} value={formData.banking_details.account_type} onChange={e => setFormData({ ...formData, banking_details: { ...formData.banking_details, account_type: e.target.value } })} className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")} placeholder="e.g. Cheque / Current" />
            </Field>
            <Field label="SWIFT / BIC Code">
              <Input readOnly={!canEdit} value={formData.banking_details.swift_code} onChange={e => setFormData({ ...formData, banking_details: { ...formData.banking_details, swift_code: e.target.value } })} className={cn(INPUT_CLS, !canEdit && "bg-muted/50 cursor-not-allowed")} placeholder="e.g. FIRNZAJJ" />
            </Field>
          </div>
        </SectionCard>
        ...
        >
          ...
          {showInviteForm && (
            <form onSubmit={handleInvite} className="mb-5">
              <p className="text-xs text-gray-500 mb-3">Invite users to your organisation.</p>
              <div className="space-y-2">
                {inviteRows.map((row) => (
                  <div key={row.id} className="flex items-start gap-2 p-3 rounded-xl bg-muted/50">
          ...
          {teamLoading ? (
            ...
            <div className="space-y-2">
              {members.map(member => (
                <div key={member.id} className="flex items-center gap-3 px-4 py-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
                  <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center shrink-0 text-xs font-normal text-primary">
                  ...
                  {member.role && (
                    <Badge variant="neutral" className="shrink-0">{member.role}</Badge>
                  )}
                  {canEdit && (
                    <button
              ...
              {pendingInvites.length > 0 && (
                <>
                  <p className="text-xs text-muted-foreground normal-case pt-3 pb-1 px-1">Pending Invitations</p>
