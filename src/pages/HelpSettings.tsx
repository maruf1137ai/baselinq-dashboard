...
export default function HelpSettings() {
  ...
  // deep link out of /settings/team-management, anywhere — rather than always dumping them on
  // /settings/team-management. history.state.idx is React Router's history index: 0 (or
  // undefined) means this page is the first entry, so there is nothing to
  // pop and we fall back to /settings/team-management.
  const goBack = () => {
    if (window.history.state?.idx > 0) navigate(-1);
    else navigate("/settings/team-management", { replace: true });
  };
