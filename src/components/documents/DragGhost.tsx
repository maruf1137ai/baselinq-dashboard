...
export function DragGhost({ doc }: { doc: ApiDocument }) {
  return (
    <div className="flex items-center gap-2 bg-card border border-primary/40 rounded-md shadow-lg px-3 py-2 max-w-xs">
      ...
      {doc.reference && (
        <span className="font-mono text-xs px-1.5 py-0.5 bg-primary/10 text-primary rounded shrink-0">
          {doc.reference}
        </span>
      )}
    </div>
