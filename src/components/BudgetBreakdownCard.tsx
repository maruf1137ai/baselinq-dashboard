...
export function BudgetBreakdownCard({ progress: propProgress, daysStatus: propDaysStatus }) {
  ...
  const dynamicBudgetData = [
    { name: 'Subcontractor Costs', value: projectTotalBudget * 0.40, percentage: 40, color: '#6c5ce7' },
          ...
          <div className="flex items-center justify-center col-span-5">
            <div className="relative w-56 h-56">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <p className="text-lg text-foreground">R {projectTotalBudget.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
            ...
            {dynamicBudgetData.map(item => (
              <div key={item.name} className="flex items-center justify-between">
                ...
                <span className="text-sm text-foreground">
                  R {item.value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} <span className="text-xs text-muted-foreground">({item.percentage}%)</span>
                </span>
              </div>
            ))}
          </div>
