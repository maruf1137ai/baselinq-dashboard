    ...
    <div className="space-y-6 mt-8">
            ...
            {priceData.items_analysis?.map((item: any, idx: number) => (
              <div key={idx} className="p-4 bg-sidebar rounded-lg border border-gray-100">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h5 className="font-medium text-[#1B1C1F]">Item {item.item_number}: {item.description}</h5>
                ...
                {item.market_verification && (
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-white rounded-md border border-gray-100">
                      <p className="text-xs font-medium text-gray-400 uppercase mb-2">Market Benchmarks (ZAR)</p>
                      <div className="flex items-end gap-2">
                        <span className="text-lg font-medium text-primary">R {item.market_verification.market_rate_range.average}</span>
                    ...
                    <div className="p-3 bg-white rounded-md border border-gray-100">
                      <p className="text-xs font-medium text-gray-400 uppercase mb-2">AI Assessment</p>
                      <div className="flex items-center gap-2">
                        <span className={`text-sm font-medium ${item.market_verification.variance_percentage > 0 ? 'text-green-600' : 'text-destructive'}`}>
                ...
                {item.market_verification?.web_search_result && (
                  <div className="mt-3 p-3 bg-indigo-50/30 rounded-md border border-indigo-100/50">
                    <p className="text-[10px] font-medium text-indigo-700 uppercase mb-1 flex items-center gap-1">
      ...
      <div className={sectionClass(visibleSections, startSelector + 1)}>
        <div className="grid grid-cols-2 gap-6">
          <div className="p-6 bg-white border border-border rounded-[14px]">
            <h4 className="text-sm font-medium text-[#0E1C2E] mb-4 flex items-center gap-2">
          ...
          <div className="p-6 bg-primary text-white rounded-[14px] shadow-lg shadow-primary/20">
            ...
            <div className="space-y-2">
              ...
              <div className="pt-2 border-t border-white/20 flex justify-between items-center font-medium">
